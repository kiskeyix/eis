...
<!-- 
Stylesheet to convert Netstat XML output to HTML

Author: Luis Mondesi <lemsx1@gmail.com>

Date: 2006-04-04 15:44 EDT 
Usage: xsltproc service.xsl netstat.xml > netstat.html
...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="/node/service">
    ...
    <xsl:template match="proto">
        <p class='protocol'>
            Protocol: <xsl:apply-templates />
        </p>
    </xsl:template>

    <xsl:template match="listenip">
        <p class='listenip'>
            Listen IP: <xsl:apply-templates />
        </p>
    </xsl:template>
