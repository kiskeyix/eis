...
package EIS::Tables::HostMeta;
use base 'EIS::DBI';
use strict;

__PACKAGE__->set_up_table("hostmeta");
__PACKAGE__->autoinflate("host" => "EIS::Tables::Host");
__PACKAGE__->has_a("host" => "EIS::Tables::Host");

sub get_hostmeta_fields
{
    my @_meta = (
                 'ip',              'ipv6',
                 'description',     'owner',
                 'businesscontact', 'techcontact',
                 'vendorcontact',   'notes',
                 'console',         'application',
                 'contract',        'expires',
                 'maintenance',
